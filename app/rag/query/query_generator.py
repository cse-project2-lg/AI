...
def generate_query_from_event(sensor_event: Dict[str, Any]) -> str:
    ...
    csi = sensor.get("csi", {}) if isinstance(sensor.get("csi"), dict) else {}
    ...
    csi_status = csi.get("status")
    csi_score = csi.get("changeScore")
    csi_packet_count = csi.get("packetCount")
    local_score = sensor_event.get("localScore")
    local_risk_level = sensor_event.get("localRiskLevel")
    candidate_reason = sensor_event.get("candidateReason") or []
    ...
    if csi_status == "AVAILABLE" and csi_score is not None:
        if csi_score >= 0.7:
    ...
    elif csi_status in {"UNAVAILABLE", "STUB"}:
        query_parts.append(f"CSI 상태는 {csi_status}이며 사유는 {csi.get('reason', '미기재')}")
    ...
    if local_risk_level:
        query_parts.append(f"엣지 로컬 위험도는 {local_risk_level}")

    if candidate_reason:
        query_parts.append("후보 사유: " + ", ".join(str(item) for item in candidate_reason))

    purpose = "낙상 후보 판단 기준, 사용자 MP3/STT 확인 절차, 보호자 클라우드 알림 대응 정책"
